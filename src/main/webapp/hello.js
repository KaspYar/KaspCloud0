...
function enableButtons () {
	...
	// Set the onclick action for the second button
	btn = document.getElementById("input_calcSum");
	btn.onclick=function(){calcSum();};

	// Update the button label now that the button is active
	btn.value="Click me to calculate sum";
}
/*
...
function calcSum () {
	...
	var csv = document.getElementById("csv_field").value;

	// Call the sayHelloByName() function.
	// It takes one argument "name"
	// On success, pass the response to sayHelloCallback()
	var request = gapi.client.helloworldendpoints.calcSum({'csv': csv});
	request.execute(sayHelloCallback);
}
